...
#             cnts.df <- read.table(cntMat.pwd)
            cnts.df <- read.csv(cntMat.pwd, row.names = 1)
...
createPB <- function(seu.obj = NULL, groupBy = "clusterID_sub", comp = "cellSource", biologicalRep = "orig.ident",
    ...
    test <- lapply(levels(seu.obj), function(x) {
        ...
        if(length(zKeep) >= 0.5*ztest){
            ...
            if(lowFilter){
                #Only keep genes that have at least 10 cells with expression > 1
...
btwnClusDEG <- function(seu.obj = NULL,groupBy = "majorID_sub", idents.1 = NULL, idents.2 = NULL, bioRep = "orig.ident",padj_cutoff = 0.01, lfcCut = 0.5, topn=c(20,20), strict_lfc = F,
                        minCells = 25, outDir = "", title = NULL, idents.1_NAME = "", idents.2_NAME = "", returnVolc = F, doLinDEG = F, paired = T, addLabs = NULL, lowFilter = F, dwnSam = T, setSeed = 12, dwnCol = "blue", stblCol = "grey",upCol = "red", labSize = 3, saveAllRes = FALSE, featuresToExclude = NULL
                    ){
    ...
    seu.sub$compare <- ifelse(seu.sub@meta.data[[groupBy]] == grepTerm, "idents.1", "idents.2")
    ...
    p <- pseudoDEG(padj_cutoff = padj_cutoff, lfcCut = lfcCut, outName = paste(gsub(" ", "_", idents.1_NAME), "_vs_",gsub(" ", "_",idents.2_NAME), sep = ""), strict_lfc = strict_lfc,
              outDir = outDir, title = title, fromFile = F, meta = meta, pbj = pbj, returnVolc = returnVolc, paired = paired, pairBy = "bioRepPair",
                   idents.1_NAME = idents.1_NAME, idents.2_NAME = idents.2_NAME, minimalOuts = T, saveSigRes = T, addLabs = addLabs, topn = topn, dwnCol = dwnCol, stblCol = stblCol,upCol = upCol, labSize = labSize, saveFullRes = saveAllRes, featuresToExclude = featuresToExclude
                     )    
    return(p)
}
############ volcFromFM ############
...
prettyVolc <- function(
    ...
    {if(arrowz){
        maxVal <- max(abs(plot$data$log2FoldChange))
        midVal <- ((lfcCut + maxVal*0.1) + maxVal) / 2
        annotate("segment", 
                 x = lfcCut+maxVal*0.1, 
                 y = y * 1.06, 
                 xend = maxVal, 
     ...
     {if(arrowz){
        annotate("segment", x = -(lfcCut+maxVal*0.1), 
                 y = y * 1.06, 
                 xend = -maxVal,
                 yend = y * 1.06, 
...
plotGSEA <- function(
    ...
    if(!is.null(filterTerm)){
        namez <- unique(can_gene_sets$gs_name)
        namez <- namez[grepl(filterTerm, namez)]
        datas <- can_gene_sets %>% 
            filter(gs_name %in% namez) %>%
...
#         enriched <- enriched_up[1:ifelse(nrow(enriched_up) > termsTOplot, termsTOplot, nrow(enriched_up)), ]
    ...
    if(trimTerm){
        ...
        enriched$Description <- gsub("GSE[^_]*_", "", enriched$Description)
        ...
        enriched$Description <- gsub("ENDOPLASMIC_RETICULUM", "ER", enriched$Description)
        if(trunkTerm){
            enriched$Description <- unlist(lapply(enriched$Description, function(x){
                cutz <- unlist(gregexpr(pattern = '_', x))
                cut_loc <- max(cutz[cutz < 50])
                if(nchar(x) > 50){
...
#             enriched$Description <- unlist(lapply(enriched$Description, str_trunc, width = 50, side = "right", ellipsis = "*")) ### TO DO: this causes problems if trunc results in the same string -- need to ensure that is handled before adding this back in
        ...
        enriched$Description <- gsub("_", " ", enriched$Description)
        dups <- enriched$Description[duplicated(enriched$Description)]
        enriched <- enriched %>% 
            separate(ID, into = c("gse"), sep = "_", remove = F) %>%
            mutate(
                Description = ifelse(Description %in% dups, 
                                     paste0(gse, " ", Description),
                                     Description
                                    )
            ) %>%
            select(-gse)
    ...
    if(!is.null(saveRes)){
            write.csv(enriched, file = saveRes)
    }
    
    if(!upOnly){
        enriched <- enriched[enriched$ID %in% c(
            enriched_up[1:ifelse(nrow(enriched_up) > termsTOplot, termsTOplot, length(enriched_up)), ]$ID, 
            enriched_dwn[1:ifelse(nrow(enriched_dwn) > termsTOplot, termsTOplot, length(enriched_up)), ]$ID
        ), ]
    } else {
        enriched <- enriched[enriched$ID %in% c(
            enriched_up[1:ifelse(nrow(enriched_up) > termsTOplot, termsTOplot, nrow(enriched_up)), ]$ID
        ), ]
    }
    ...
    if(!lolli){
        ...
        geom_text(aes(x = ifelse(x_axis > 0, - 0.1 * max(enriched$x_axis), 0.1 * max(enriched$x_axis)), label = Description), hjust = ifelse(enriched$x_axis > 0, 1,0), size = size) + 
        theme(axis.ticks.y = element_blank(),
              axis.text.y = element_blank(),
              axis.title.y = element_blank(),
              axis.line.y = element_blank(),
              legend.justification = "top"
             ) + coord_cartesian(clip = "off") + scale_x_symmetric(mid = 0, name = "Signed log10(p.adj)") + NoLegend() + scale_fill_manual(values = c(upCol,dwnCol))
        } else{
            p <- ggplot(data = enriched, aes(x = x_axis, y = Description, colour = direction, size = Count)) +
                ...
                geom_text(aes(x = ifelse(x_axis > 0, - 0.1 * max(enriched$x_axis), 0.1 * max(enriched$x_axis)), label = Description), 
                ...
                scale_x_symmetric(mid = 0, name = "Signed log10(p.adj)") + 
...
crossSpeciesDEG <- function(
    ...
    colUp = "red",
    colDwn = "blue",
    cONtrast = c("celltype_1","celltype_2"),
    seed = 12,
    ...
    geneLists.spec1 <- geneLists.spec1 %>% 
        mutate(
            species = species1,
            signed_pVal_spec1 = ifelse(log2FoldChange > 0, 
                                       -log10(padj), 
                                       log10(padj)),
            log2FoldChange_spec1 = log2FoldChange
        ) 
    geneLists.spec2 <- geneLists.spec2 %>% 
        mutate(
            species = species2,
            signed_pVal_spec2 = ifelse(log2FoldChange > 0, 
                                       -log10(padj), 
                                       log10(padj)),
            log2FoldChange_spec2 = log2FoldChange
        ) 
    if(useFC) {
        geneLists.spec1$spec1 <- geneLists.spec1$log2FoldChange_spec1
        geneLists.spec2$spec2 <- geneLists.spec2$log2FoldChange_spec2
        axis_label = "log2FC"
    } else {
        geneLists.spec1$spec1 <- geneLists.spec1$signed_pVal_spec1
        geneLists.spec2$spec2 <- geneLists.spec2$signed_pVal_spec2
    }
    geneList <- as.data.frame(unique(c(
        geneLists.spec1$gene, geneLists.spec2$gene
    )))
    colnames(geneList) <- "gene"
    
    #add intersect to only plot overlappling genes btwn species -- done above?
    geneList <- geneList %>% 
        left_join(geneLists.spec1[,c("gene","species","spec1", 
                                     "log2FoldChange_spec1", "signed_pVal_spec1")], 
                  by = "gene") %>% 
        left_join(geneLists.spec2[,c("gene","spec2", 
                                     "log2FoldChange_spec2", "signed_pVal_spec2")], 
                  by = "gene") %>% 
        replace(is.na(.), 0)
   
#     #add if
#     geneList <- geneList %>%
#         filter(abs(signed_pVal_spec1) > 2 | abs(signed_pVal_spec2) > 2)
    bothSpecies <- intersect(geneLists.spec1$gene, geneLists.spec2$gene)
    spec1Only <- geneLists.spec1$gene[! geneLists.spec1$gene %in% bothSpecies]
    spec2Only <- geneLists.spec2$gene[! geneLists.spec2$gene %in% bothSpecies]
    geneList <- geneList %>% 
        mutate(
            species = case_when(
                gene %in% bothSpecies ~ "Both",
                gene %in% spec1Only ~ species1,
                gene %in% spec2Only ~ species2
            ),
            sig = ifelse(spec2 * spec1 == 0,
                         spec2 + spec1,
                         spec2 * spec1),
            direction = case_when(
                spec2 * spec1 != 0 & spec1 > 0 & spec2 > 0 ~ "up",
                spec2 * spec1 != 0 & spec1 < 0 &  spec2 < 0 ~ "dwn",
                spec2 * spec1 == 0 & spec1 > 0 ~ "axis1",
                spec2 * spec1 == 0 & spec2 > 0 ~ "axis2",
                spec2 * spec1 == 0 & spec1 < 0 ~ "axis3",
                spec2 * spec1 == 0 & spec2 < 0 ~ "axis4",                             
                spec2 * spec1 != 0 & spec1 > 0 & spec2 < 0 ~ "conflict1",
                spec2 * spec1 != 0 & spec1 < 0 & spec2 > 0 ~ "conflict2"
            ),
        ...
        mutate(
            ...
            lab = ifelse(row_number() <= nlab & grepl("up|dwn", direction), 
                         gene,
                         ifelse(grepl("axis", direction) & row_number() <= nlab_axis, 
                                gene, 
                                ifelse(grepl("conflict", direction) & row_number() <= nlab_conflict, 
                                       gene, 
                                       NA)
                               )
                        )
        ...
        mutate(
            direction = ifelse(abs(signed_pVal_spec1) < 2 & abs(signed_pVal_spec2) < 2,
                               "n.s.",
                               direction),
            lab_col = factor(
                ifelse(abs(signed_pVal_spec1) < 2 & abs(signed_pVal_spec2) < 2,
                       "grey",
                       lab_col),
                levels = c("grey", "black", "blue", "red", "hotpink")
            ),
            lab = ifelse(abs(signed_pVal_spec1) < 2 & abs(signed_pVal_spec2) < 2,
                               NA,
                               lab)
        ) %>%
        arrange(lab_col)
    spacer <- 1.3
    max_x <- max(abs(geneList$spec1)) * spacer
    max_y <- max(abs(geneList$spec2)) * spacer
    anno.df <- as.data.frame(list(
        direction = c("axis1","axis2","axis3","axis4","conflict1","conflict2","up","dwn"),
        xpos = c(max_x,0,-max_x,0,max_x,-max_x,max_x,-max_x),
        ypos =  c(0,max_y,0,-max_y,-max_y,max_y,max_y,-max_y),
        hjustvar = hjustvar,
        vjustvar = vjustvar,
    ...
    if(saveGeneList){
        write.csv(arrange(geneList, desc(sig)), 
                  file = paste0(outDir, outName, "_", species1, "_v_", species2, "_", 
                                cONtrast[1], "_", cONtrast[2], ".csv"), 
                  row.names = F)
    }
    p <- ggplot(geneList, aes(x = spec1, y = spec2)) + 
        geom_hline(yintercept=0,linetype=2) +
        geom_vline(xintercept=0,linetype=2) + 
        geom_point(color = geneList$lab_col, shape = 20, alpha = 0.5) + 
        ...
        labs(
            x = paste0(species1, " " , cONtrast[1], " versus ", cONtrast[2],
                       " (", axis_label, ")"), 
            y = paste0(species2, " ", cONtrast[1], " versus ", cONtrast[2],
                       " (", axis_label, ")"), 
            title = paste0(species1, " and ", species2," " , cONtrast[1]," vs ",
                           cONtrast[2])) + 
        coord_cartesian(clip = 'off') + 
...
skewPlot <- function(
    ...
    if(!is.null(groupBy)){
        ...
        #get data
        pct.df <- table(
            seu.obj@meta.data[[groupBy]], seu.obj@meta.data[[sampleRep]]) %>% 
            melt() %>% 
            group_by(Var.2) %>% 
            mutate(samN = sum(value))
        ...
        statz <- compare_means(pct ~ cellSource, group.by = "Var.1", pct.df) %>%
            select(-p.format, -.y.) %>%
            mutate(
                p.signif = symnum(p, cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, Inf),
                                  symbols = c("****", "***", "**", "*", "ns"))
            )
        ...
        status <- pct.df %>% 
            ...
            left_join(statz[c("Var.1","p")], by = "Var.1") %>%
            mutate(
                lfc = abs(log2(Tumor/Blood)), #hard coded
                lab_col=case_when(lfc >= 3 & p < 0.05 ~ "Unique",
                                  p < 0.05 & lfc < 3 ~ "Skewed",
                                  p > 0.05 ~ "Common")) %>% 
        ...
        p <- ggplot(pct.df, aes(x = Var.1, y = pct)) +
            geom_boxplot(aes(x = Var.1, fill = cellSource), alpha = 0.25, outlier.shape = NA, outlier.alpha = 0) + 
            geom_point(
                aes(fill = cellSource, group = cellSource), 
...
sigDEG_heatmap <- function(
    ...
    pd <- packLegend(lgd1, lgd2, max_width = unit(45, "cm"), 
        direction = "vertical", column_gap = unit(5, "mm"), row_gap = unit(0.5, "cm"))
    ...
    par(mfcol=c(1,1))   
    draw(ht, padding = unit(c(2, 2, 2, 7), "mm"), heatmap_legend_side = "bottom", 
         annotation_legend_list = pd, annotation_legend_side = "right")

    for(i in 1:nClus){
        decorate_annotation("Cluster", slice = i, {
            grid.text(paste0("c", (1:nClus) - 1)[i], just = "center", gp = gpar(col = font_colz[i]))
        })
    }
    dev.off()
    return(ht)
